...
- **`src/studyTracker.jsx`** — root component. Holds all persisted state via the `usePersistedState(loader, saver)` helper and renders the active view.
  - **Tracks:** the `tracker` view supports multiple plans via the `TRACKS` registry (`main` = `MAIN_PLAN` [49-day + `QUANT_PLAN`]; `hrt` = `HRT_PLAN` from `data/hrtPlan.js`). The selected track lives in the `track` slice; each track has its OWN done-flags storage key — `KEYS.tracker` for main, `KEYS.trackerHrt` (`hrt_tracker_v1`) for HRT — so progress never collides. The component keeps `mainDone` and `hrtDone` as separate persisted states and picks `done`/`setDone` by active track. **Today and Dashboard always use `MAIN_PLAN` + `mainDone`** (the track switch only affects the Tracker view). Task ids are `d{day.d}t{index}` within each plan independently. New categories `dp` (Greedy & DP) and `format` (Format Rehearsal) were added to `CATS`; `prob` already existed. To add a track: add an entry to `TRACKS`, a week-titles map, and (if its done-flags should be publishable) a storage slice wired into export/import/reset + `progress.json`.
