            ...
            {activity.bars.map((b) => (
              <div key={b.k} title={`${b.k}: ${b.n}`} style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "flex-end", height: "100%" }}>
                <div style={{ height: `${(b.n / activity.max) * 100}%`, minHeight: b.n ? 4 : 2,
                  background: b.n ? C.blue : C.subtle, borderRadius: 4 }} />
              </div>
            ))}
          </div>
...
const H = ({ children }) => (
  <div style={{ fontSize: 13.5, fontWeight: 700, color: C.text, marginBottom: 12, fontFamily: C.sys }}>{children}</div>
);
const Dim = ({ children }) => (
...
function Bar({ label, value, total, color, compact }) {
  ...
  return (
    <div style={{ marginBottom: compact ? 8 : 12 }}>
      ...
      <div style={{ height: compact ? 6 : 8, background: C.subtle, borderRadius: 5, overflow: "hidden" }}>
