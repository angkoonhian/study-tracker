  ...
  sections: [
    ...
    },
    {
      ...
      blocks: [
        ...
        { type: "diagram", kind: "array", data: { values: [2, 1, 3], pointers: [{ name: "i", index: 1, color: "#2f8d46" }], highlight: [], labels: {} }, caption: `Step A (i=1, nums=[2,1,3]). nums[i]=1 is NOT greater than the stack top's value (nums[0]=2), so nothing pops; we just push index 1. Stack holds [0, 1] with values 2, 1 — DECREASING bottom->top (the invariant).` },
        ...
        { type: "diagram", kind: "array", data: { values: [2, 1, 3], pointers: [{ name: "i", index: 2, color: "#2f8d46" }], highlight: [0, 1], labels: { "0": "ans=3", "1": "ans=3" } }, caption: `Step B (i=2). nums[i]=3 is greater than the top (1) and then the next (2): both pop. Each popped index just found its NEXT GREATER = 3, so ans[1]=3 and ans[0]=3 (highlighted, resolved). Then push index 2; it stays unresolved (ans=-1).` },
    ...
    },
    {
      ...
      blocks: [
        ...
        { type: "diagram", kind: "array", data: { values: [73, 74, 72, 76], pointers: [{ name: "i", index: 2, color: "#2f8d46" }], highlight: [0], labels: { "0": "ans=1" } }, caption: `temps=[73,74,72,76]. At i=2 (temp 72): 72 < stack top temp (74), so nothing pops; push index 2. Index 0 was already resolved earlier (74 at i=1 warmed it after 1 day, ans[0]=1, highlighted). Stack now holds indices [1, 2] (temps 74, 72 — decreasing).` },
        ...
        { type: "diagram", kind: "array", data: { values: [73, 74, 72, 76], pointers: [{ name: "i", index: 3, color: "#2f8d46" }], highlight: [0, 1, 2], labels: { "1": "ans=2", "2": "ans=1" } }, caption: `At i=3 (temp 76): 76 beats top 72 -> pop index 2, ans[2]=3-2=1 day; 76 beats 74 -> pop index 1, ans[1]=3-1=2 days. Answer is the DISTANCE i-j, which is why we store indices. Index 3 is pushed with no warmer day after it, so ans[3]=0.` },
