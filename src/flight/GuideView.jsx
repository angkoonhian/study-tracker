    ...
    <div style={{ display: "grid", gridTemplateColumns: "230px minmax(0,1fr) 210px", gap: 16, alignItems: "start" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          {GUIDES.map((g) => (
            <button key={g.id} onClick={() => { setId(g.id); window.scrollTo({ top: 0 }); }} style={{
              textAlign: "left", background: g.id === id ? C.chipBg : "transparent",
      ...
      <div style={{ minWidth: 0 }}>
        <div style={{ marginBottom: 16 }}>
          <h1 style={{ margin: "0 0 4px", fontSize: 30, color: C.strong }}>{guide.title}</h1>
        ...
        {guide.sections.map((s, i) => (
          <section key={i} ref={(el) => (sectionRefs.current[i] = el)} style={{ scrollMarginTop: 64, marginBottom: 22 }}>
            <h2 style={{ fontSize: 22, color: C.strong, margin: "0 0 12px",
        ...
        {guide.cheatsheet && guide.cheatsheet.length > 0 && (
          <Panel style={{ background: C.chipBg, borderColor: C.green, marginBottom: 30 }}>
...
function Block({ b }) {
  switch (b.type) {
    ...
    case "h3":
      return <h3 style={{ fontSize: 16.5, color: C.text, margin: "18px 0 8px" }}>{b.text}</h3>;
    case "ul":
    ...
    case "code":
      return (
        <pre style={{ background: C.soft, border: `1px solid ${C.border}`, borderRadius: 10,
          padding: "13px 15px", overflowX: "auto", margin: "10px 0", fontFamily: mono,
          fontSize: 12.8, lineHeight: 1.55, color: C.text, whiteSpace: "pre" }}>{b.code}</pre>
      );
    case "callout":
      return (
        <div style={{ borderLeft: `3px solid ${C.blue}`, background: C.chipBg,
          borderRadius: "0 8px 8px 0", padding: "10px 14px", margin: "12px 0",
          fontFamily: C.sys, fontSize: 13.5, color: C.text, lineHeight: 1.6 }}>{b.text}</div>
      );
    case "table":
  ...
  return (
    <div style={{ overflowX: "auto", margin: "12px 0" }}>
      <table style={{ borderCollapse: "collapse", width: "100%", fontFamily: C.sys, fontSize: 13 }}>
        ...
        <tbody>
          {rows.map((r, i) => (
            <tr key={i} style={{ background: i % 2 ? "transparent" : C.pageBg }}>
              {r.map((cell, j) => (
